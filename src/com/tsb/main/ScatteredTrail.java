...
public class ScatteredTrail extends Trail {
    ...
    public ScatteredTrail(int x, int y, ID id, Handler handler, Color color, int width, int height, float life, boolean allowedTrail) {
    ...
    public void tick(){
        ...
        if ( alpha < 0.70 && allowedTrail){
            allowedTrail = false;
            handler.addObject(new ScatteredTrail(this.x, this.y, this.id, handler, new Color(83, 21, 24), random.nextInt(10), random.nextInt(10), 0.01f, false));
        }
